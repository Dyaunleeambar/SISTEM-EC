@echo off
title Creando Ejecutable Definitivo con Frontend
color 0A

echo ========================================
echo    Creando Ejecutable Definitivo
echo    con Frontend Integrado
echo ========================================
echo.

REM Crear un archivo temporal con configuración correcta
echo Creando server-definitivo.js...
(
...
echo.
echo // SERVIR ARCHIVOS ESTÁTICOS DESDE LA CARPETA APP
...
echo     console.log(''^);
echo     console.log('ARCHIVOS FRONTEND SERVIDOS DESDE: ' + path.join(__dirname, 'app'^)^);
echo     console.log('RUTA PRINCIPAL: http://localhost:' + PORT + '/'^);
echo     console.log(''^);
...
echo }
echo.
echo // RUTA PRINCIPAL - SERVIR INDEX.HTML
echo app.get('/', (req, res^) => {
echo   const indexPath = path.join(__dirname, 'app', 'index.html'^);
echo   console.log('Solicitud a ruta principal, sirviendo:', indexPath^);
echo   if (fs.existsSync(indexPath^)) {
echo     console.log('Archivo index.html encontrado, enviando...'^);
echo     res.sendFile(indexPath^);
echo   } else {
echo     console.log('ERROR: Archivo index.html NO encontrado en:', indexPath^);
echo     res.status(404).send('Archivo index.html no encontrado en la carpeta app'^);
echo   }
echo });
echo.
echo // Ruta para verificar que el servidor está funcionando
echo app.get('/api/health', (req, res^) => {
echo   res.json({ status: 'OK', message: 'Servidor funcionando correctamente' });
echo });
echo.
...
echo Archivo server-definitivo.js creado.
echo.

REM Crear el ejecutable definitivo
echo Creando ejecutable definitivo con frontend...
pkg server-definitivo.js --target node18-win-x64 --output SistemaEstimulacionDefinitivo.exe

if %errorlevel% equ 0 (
    echo.
    echo ========================================
    echo    Ejecutable Definitivo creado!
    echo ========================================
    echo.
    echo Archivo: SistemaEstimulacionDefinitivo.exe
    echo.
    echo Características:
    echo - Manejo correcto de archivos frontend
    echo - Servir archivos estáticos desde carpeta app
    echo - Ruta principal configurada correctamente
